...
class ERRNetBase(BaseModel):
    ...
    def set_input(self, data, mode='train'):
        ...
        if mode == 'train':
            ...
            input, data_name,target_t = data['input'], data['fn'], data['target_t']
        else:
        ...
        if len(self.gpu_ids) > 0:  # transfer data into gpu
            ...
            # TODO：test
            # input = input.to(device=self.device0)

            if target_t is not None:
                # TODO：train
                target_t = target_t.to(device=self.gpu_ids[0])
                # TODO：test
                # target_t = target_t.to(device=self.device0)

            if target_r is not None:
                # TODO：train
                target_r = target_r.to(device=self.gpu_ids[0])     
                # TODO：test
                # target_r = target_r.to(device=self.device0)            
    ...
    def test(self, data, savedir=None):
        ...
        if self.data_name is not None and savedir is not None:
            ...
            if os.path.exists(join(savedir, name, '{}.png'.format(self.opt.name))):
                print("%s is exist"%(join(savedir, name, '{}.png'.format(self.opt.name))))
                return 
        
        with torch.no_grad():
            # print("self.target.shape",self.target_t.shape)
            output_i = self.forward()

            if 'cdr' in savedir:
                _, gt_C, gt_H, gt_W = self.target_t.shape
                # 1280 640 320
                # 3840 1920 960
                if gt_W < 640:
                    paddingw = 960 - gt_W
                ...
                output_i = output_i[..., paddingh:paddingh+gt_H, paddingw:paddingw+gt_W]
                self.input = self.input[..., paddingh:paddingh+gt_H, paddingw:paddingw+gt_W]
            ...
            if self.data_name is not None and savedir is not None:                
                Image.fromarray(output_i.astype(np.uint8)).save(join(savedir, name, '{}.png'.format(self.opt.name)))
                Image.fromarray(tensor2im(self.input).astype(np.uint8)).save(join(savedir, name, 'm_input.png'))
                Image.fromarray(tensor2im(self.target_t).astype(np.uint8)).save(join(savedir, name, 'm_gt.png'))


            fakeT = cv2.imread(join(savedir, name, '{}.png'.format(self.opt.name)), -1)
            # print(fakeT.shape)
            realT = cv2.imread(join(savedir, name, 'm_gt.png'), -1)
        ...
        return name, result_psnr, result_ssim
...
class ERRNetModel(ERRNetBase):
    ...
    def initialize(self, opt):
        ...
        if(len(opt.gpu_ids) <= 0):
            ...
            # TODO：train
            self.device = torch.device(self.gpu_ids[0] if torch.cuda.is_available() else "cpu")
            # TODO：test
        ...
        if opt.hyper:
            ...
            # TODO：test
            # self.vgg = losses.Vgg19(requires_grad=False).to(self.device0)
            in_channels += 1472
        ...
        # TODO：test
        # self.edge_map = EdgeMap(scale=1).to(self.device0)
    ...
    def forward(self):
        # TODO：train
        input_i = self.input.to(self.device)
        ...
        output_i = self.net_i(input_i.to(self.device))
        self.output_i = output_i
        
        # TODO：test
        # input_i = self.input.to(self.device0)
        # if self.vgg is not None:
        #     hypercolumn = self.vgg(self.input)
        #     _, C, H, W = self.input.shape
        #     hypercolumn = [F.interpolate(feature.detach(), size=(H, W), mode='bilinear', align_corners=False) for feature in hypercolumn]
        ...
        #     input_i.extend(hypercolumn)
        #     input_i = torch.cat(input_i, dim=1)
        # output_i = self.net_i(input_i.to(self.device1))
        # self.output_i = output_i
