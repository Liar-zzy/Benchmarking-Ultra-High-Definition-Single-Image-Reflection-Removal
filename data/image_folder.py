...
def make_dataset_uhd(dir, opt, phase, max_dataset_size=float("inf")):
    # for uhd train
    ...
    json_path = os.path.join(dir,opt.uhd+'_'+phase+'_results_list.json')
    ...
    blended_paths[:min(max_dataset_size, len(blended_paths))],alpha_list[:min(max_dataset_size, len(alpha_list))]

def make_dataset_uhd_crop(dir, opt, max_dataset_size=float("inf")):
    # for 8k test cropped
    reflection_mask_paths = []
    trans_paths = []
    blended_paths = []
    ...
    json_path = os.path.join(dir,'cropped',opt.uhd+'_test_crop_results_list.json')

    with open(json_path,'r') as load_f:     
        input_list = json.load(load_f)
    load_f.close()

    reflect_mask_root = os.path.join(dir, 'cropped', 'reflection_mask_layer')
    # reflect_root = os.path.join(dir, 'reflection_layer')
    trans_root = os.path.join(dir, 'cropped', 'transmission_layer')
    blend_root = os.path.join(dir, 'cropped', 'blended')
